...
def render_markdown(
    ...
    if not paths:
        ...
        if outbound_control:
            lines.extend(_outbound_control_md(G, outbound_control))
        if intermediate:
...
def _outbound_control_md(G: "nx.DiGraph", controls: list[dict]) -> list[str]:
    direct_count = sum(1 for e in controls if e["via_group"] is None)
    lines: list[str] = [
        "\n## Outbound object control\n",
        f"{direct_count} direct privilege(s), {len(controls) - direct_count} via group.\n",
        "| Relation | Target | How |",
        "|----------|--------|-----|",
    ]
    for e in controls:
        ...
        lines.append(
            f"| **{e['relation']}** | `{_display_name(G, e['dst'])}` | {via} |"
        )
    lines.append("")
    return lines
